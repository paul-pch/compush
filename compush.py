...
def run_git_command(command: list[str], display: bool = True) -> tuple[bool, str]:
    """Exécute une commande git et retourne (succès, output)"""
    try:
        result = subprocess.run(command, check=True, capture_output=True, text=True)
        # git écrit souvent ses infos (ex: push) sur stderr même en cas de succès
...
def get_current_branch() -> str:
    ...
    success, output = run_git_command(["git", "rev-parse", "--abbrev-ref", "HEAD"], display=False)
...
def main(
    ...
    success, output = run_git_command(["git", "status", "--porcelain"], display=False)
    if not success:
    ...
    if current_branch in ["master", "main"] and not master:
        ...
        success, output = run_git_command(["git", "checkout", "-b", new_branch])
        if not success:
            console.print(f"[bold red]Erreur lors de la création de la branche: {output}[/bold red]")
            sys.exit(1)

    elif branch and branch != current_branch:
        console.print(f"\n[bold]🔀 Changement de branche: {branch}[/bold]")
        success, output = run_git_command(["git", "checkout", "-b", branch])
        if not success:
            console.print(f"[bold red]Erreur lors de la création de la branche: {output}[/bold red]")
            sys.exit(1)
    ...
    if not success:
        console.print(f"[bold red]Erreur git add: {output}[/bold red]")
        sys.exit(1)
    ...
    if not success:
        console.print(f"[bold red]Erreur git commit: {output}[/bold red]")
        sys.exit(1)

    # Vérifier si un remote existe
    success, remotes = run_git_command(["git", "remote"], display=False)
    ...
    if not success:
        ...
        current_branch = get_current_branch()
        success, output = run_git_command(["git", "push", "-u", "origin", current_branch])

        if not success:
            console.print(f"[bold red]🚨 Erreur - Problème lors du push du code: {output}[/bold red]")
            sys.exit(1)
